...
def policy_probabilities(logit):
    #TODO: Compute probabilities of taking actions a by using the outputs of actor NN (the logits)
    #take action with higher absolute probability
    action = 0 if logit[0][0] > logit[0][1] else action = 1
    return action
...
class PPO_Agent:
    def __init__(self, no_of_states, no_of_actions):
        ...
        self.clip_ratio = 0.2 # used in original paper
    ...
    def select_action(self,state):
        ...
        action = policy_probabilities(logit)
        return action
    ...
    def nn_model(self,state_size,output_size):
        ...
        if output_size == 1:
            ...
            # ...might insert more layers later
            self.out = tf.keras.layers.Dense(output_size, activation=None)
        else:
            # actor
            self.d1 = tf.keras.layers.Dense(state_size, activation='relu')
            # ...might insert more layers later
